...
export default async function AboutPage() {
  ...
  const { company, stats, equipment } = db;
    ...
    <>
      ...
      <main className="min-h-screen font-inter">
          ...
          <div className="container mx-auto">
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
              ...
              <div className="lg:col-span-6 space-y-8">
                ...
                <h3 className="text-4xl font-bold text-on-surface leading-tight tracking-tight">{company.name}</h3>
        ...
        <section className="bg-surface-container-low py-20 px-8">
          <div className="container mx-auto grid grid-cols-2 md:grid-cols-4 gap-8">
            {stats.map((item) => (
              <div key={item.id} className="flex flex-col items-start p-8 bg-surface-container-lowest rounded-xl">
                <span className="text-5xl font-black text-primary tracking-tighter">{item.value}</span>
                <span className="text-[0.65rem] font-bold text-secondary uppercase mt-2 tracking-widest">{item.label}</span>
              </div>
            ))}
            {/* If stats are fewer than 4, show fallbacks to keep the 4-column layout filled */}
            {stats.length < 4 && Array.from({ length: 4 - stats.length }).map((_, idx) => {
              const fallbacks = [
                { value: "ISO-9001", label: "Quality Standards" },
                { value: "0.0", label: "Safety Incidents" }
              ];
              const fb = fallbacks[idx % fallbacks.length];
              return (
                <div key={`fb-${idx}`} className="flex flex-col items-start p-8 bg-surface-container-lowest rounded-xl">
                  <span className="text-5xl font-black text-primary tracking-tighter">{fb.value}</span>
                  <span className="text-[0.65rem] font-bold text-secondary uppercase mt-2 tracking-widest">{fb.label}</span>
                </div>
              );
            })}
          </div>
      ...
      <footer className="w-full py-12 px-8 flex flex-col md:flex-row justify-between items-center bg-[#f2f4f6] dark:bg-slate-900 font-inter text-xs tracking-normal">
        <div className="flex flex-col items-center md:items-start gap-2 mb-6 md:mb-0">
          <span className="font-bold text-[#00236f] text-sm uppercase tracking-tighter">{company.name}</span>
          <p className="text-[#505f76] opacity-80">© 2024 {company.name}. Industrial Integrity.</p>
        </div>
